...
[[inputs.socket_listener]]
  ...
  ##   custom header   -- split messages by recognizing a custom header at the
  ##                      start of each message. The header is described in
  ##                      `splitting_custom_header`.
  ...
  ## Specification of a custom message header used to split messages. A message
  ## boundary is found by recognizing this header at the start of a message.
  ## At least one of `const_bytes`, `length_field` or `checksum` must be given
  ## so message boundaries can be determined. The specification contains the
  ## following fields:
  ##  const_bytes  -- map of header offsets (in bytes) to constant byte values
  ##                  in hex-format that must match for a header to be valid,
  ##                  e.g. {"0" = "0xAA", "1" = "0x55"}. Values are
  ##                  case-insensitive and may use a "0x"/"x" prefix or none.
  ##  length_field -- length field describing where a message ends, using the
  ##                  same fields as `splitting_length_field` (offset, bytes,
  ##                  endianness, header_length). If omitted, message ends are
  ##                  found by scanning for the next header instead.
  ##  checksum     -- header checksum used to validate a header. It contains:
  ##                    strategy -- one of "crc32", "md5", "sha256" or "xor"
  ##                    range    -- [start, end] byte range the checksum covers
  ##                    offset   -- start of the checksum field in bytes
  ##                    bytes    -- length of the checksum field in bytes
  ## Note: This setting is only used for splitting_strategy = "custom header".
  # splitting_custom_header = {const_bytes = {}, length_field = {offset = 0, bytes = 0, endianness = "be", header_length = 0}, checksum = {strategy = "", range = [], offset = 0, bytes = 0}}
